...
def _parse_args():
    ...
    ############################
    # Modified by wan_trainer. #
    ############################
    parser.add_argument("--high_noise_lora_path", type=str, default=None, help="The path to the high_noise lora.")
    parser.add_argument("--low_noise_lora_path", type=str, default=None, help="The path to the low_noise lora.")
    parser.add_argument("--high_noise_1_lora_path", type=str, default=None, help="The path to the high_noise_1 lora.")
    parser.add_argument("--high_noise_2_lora_path", type=str, default=None, help="The path to the high_noise_2 lora.")
    parser.add_argument("--low_noise_3_lora_path", type=str, default=None, help="The path to the low_noise_3 lora.")
    parser.add_argument("--low_noise_4_lora_path", type=str, default=None, help="The path to the low_noise_4 lora.")
    parser.add_argument("--data_file", type=str, default=None, help="JSON file for inference.")
    parser.add_argument("--output_dir", type=str, default="output", help="Output directory.")
    parser.add_argument("--pattern", type=str, default=None, help="Pattern to filter data.")
    parser.add_argument("--max_gen_num", type=int, default=-1, help="Max generation number.")
...
def _init_logging(rank):
    ...
    if rank == 0:
        ...
        logging.basicConfig(
            ...
            #################################################################################################
            # Modified by wan_trainer to override the logging handlers added by some third-party libraries. #
            #################################################################################################
            force=True,
        )
    else:
...
def generate(args):
    ...
    if "t2v" in args.task:
        ################################################################
        # Modified by wan_trainer to support more flexible evaluation. #
        ################################################################
        ...
        for i, d in enumerate(data):
            cnt += 1
            if args.max_gen_num > 0 and cnt > args.max_gen_num:
                break

            output_path = os.path.join(args.output_dir, f"{i}.mp4")
        ...
        if "flash" in args.task:
            logging.info("Creating WanI2VFlash pipeline.")
            inferencer = wan.WanI2VFlash(
                **shared_kwargs,
                high_noise_1_lora_path=args.high_noise_1_lora_path,
                high_noise_2_lora_path=args.high_noise_2_lora_path,
                low_noise_3_lora_path=args.low_noise_3_lora_path,
                low_noise_4_lora_path=args.low_noise_4_lora_path,
            )
        else:
            logging.info("Creating WanI2V pipeline.")
            inferencer = wan.WanI2V(
                **shared_kwargs,
        ...
        cnt = 0
        for d in data:
            first_frame_path = d.get("first_frame_path", None)
            key_frame_path = d.get("key_frame_path", None)
            key_frame_index = d.get("key_frame_index", None)
            last_frame_path = d.get("last_frame_path", None)
            prompt = d["prompt"]

            frame_path = first_frame_path or key_frame_path or last_frame_path
            if args.pattern and not re.match(args.pattern, frame_path):
                continue
            ...
            output_filename = os.path.splitext("/".join(frame_path.split("/")[-2:]))[0] + ".mp4"
            ...
            if first_frame_path:
                ...
                kwargs.update(img=first_frame)
                message += f"first frame: {first_frame_path}, "
            if key_frame_path:
                key_frame = Image.open(key_frame_path).convert("RGB")
                kwargs.update(key_frame=key_frame, key_frame_index=key_frame_index)
                message += f"key frame: {key_frame_path}, "
            if last_frame_path:
                last_frame = Image.open(last_frame_path).convert("RGB")
                kwargs.update(last_frame=last_frame)
                message += f"last frame: {last_frame_path}, "
            message += f"prompt: {prompt}, output: {output_path}"
            logging.info(message)

            video = inferencer.generate(
                prompt,
                **kwargs,
                max_area=MAX_AREA_CONFIGS[args.size],
                frame_num=args.frame_num,

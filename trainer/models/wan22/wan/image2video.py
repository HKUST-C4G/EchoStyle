...
class WanI2V:
    ####################################################
    # Modified by wan_trainer to support loading LoRA. #
    ####################################################
    def __init__(
        self,
        config,
        ...
        if low_noise_lora_path:
            load_lora(self.low_noise_model, low_noise_lora_path)
        ...
        if high_noise_lora_path:
            load_lora(self.high_noise_model, high_noise_lora_path)
    ...
    ###########################################################
    # Modified by wan_trainer to support lf2v / flf2v / kf2v. #
    ###########################################################
    def generate(
        self,
        input_prompt,

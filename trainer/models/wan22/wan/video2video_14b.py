...
class WanV2V:
    ...
    def __init__(
        ...
        if low_noise_lora_path:
            load_lora(self.low_noise_model, low_noise_lora_path, lora_rank=64)
        ...
        if high_noise_lora_path:
            load_lora(self.high_noise_model, high_noise_lora_path, lora_rank=64)
    ...
    def generate(
        ...
        print("Using guide_scale:", guide_scale)
        print("input_video", input_video[0].shape)

        #F = frame_num
        F = input_video[0].shape[1]
        src_latent = self.vae.encode(input_video)[0]
        lat_h, lat_w = src_latent.shape[2:]
        ...
        # msk[:, 1:] = 0
        msk = torch.zeros(4, (F - 1) // self.vae_stride[0] + 1, lat_h, lat_w, device=self.device)
        ...
        y = torch.concat([msk, src_latent])
        print("y shape:", y.shape)
        #ero_y = torch.zeros_like(y, device=self.device)
    ...
    def generate_extend(
        ...
        F = input_video[0].shape[1]
        src_latent = self.vae.encode(input_video)[0]
        lat_t, lat_h, lat_w = src_latent.shape[1:]
        h = lat_h * self.vae_stride[1]
        w = lat_w * self.vae_stride[2]
        ...
        if idx == 0:
           msk = torch.zeros(4, (F - 1) // self.vae_stride[0] + 1, lat_h, lat_w, device=self.device)
            ...
            msk = torch.zeros(1, (lat_t-1)*4+1, lat_h, lat_w, device=self.device)
            msk[:, :16] = 1.0

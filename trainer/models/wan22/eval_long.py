...
def get_parser():
    ...
    parser.add_argument("--high_extend_path", type=str, default=None, help="Path to LoRA weights (.pth).")
    parser.add_argument("--low_extend_path", type=str, default=None, help="Path to LoRA weights (.pth).")
    parser.add_argument("--instruct", type=str, default=None, help="instruct")
...
def main():
    ...
    # ========== 加载模型 ==========
    try:
        wan_v2v_extend = wan.WanV2V(
            config=cfg,
            checkpoint_dir=args.ckpt_dir,
            high_noise_lora_path=args.high_extend_path,
            low_noise_lora_path=args.low_extend_path,
            device_id=device,
            rank=rank,
        ...
        wan_v2v_extend.high_noise_model.eval().requires_grad_(False)
        wan_v2v_extend.low_noise_model.eval().requires_grad_(False)
    ...
    prev = torch.zeros((3,16,480,832), device=device).contiguous() # 初始化

    for idx, d in enumerate(data):
        try:
            src_video = get_vace_data(d["src_video_path"], args.frame_num, device)
            print("src_latent loaded", d["src_video_path"], src_video[0].shape)
            prompt = args.instruct+d["prompt"]
            basename = os.path.splitext(os.path.basename(d["src_video_path"]))[0]
            output_filename = f"{basename}.mp4"
            output_path = os.path.join(args.output_dir, output_filename)

            if idx != 0:
                src_video[0][:, 0:16] = prev

            gen_clip = wan_v2v_extend.generate_extend(
                input_prompt=prompt,
            ...
            if rank == 0:
                ...
                # else:
                #     result = torch.cat([result, gen_clip[:, 16:]], dim=1)  # 拼接，去掉重复帧

                prev = gen_clip[:, -16:].contiguous()
                print("prev shape:", prev.shape)
                del gen_clip
                torch.cuda.empty_cache()  

        except Exception as e:
            logging.error(f"Failed to process sample {idx} ({d['src_video_path']}): {e}", exc_info=True)
            continue

        if dist.is_initialized():
            dist.barrier()
            dist.broadcast(tensor=prev, src=0)
    
    # if rank == 0:
    #     full_output_filename = f"full_video.mp4"
    #     full_output_path = os.path.join(args.output_dir, full_output_filename)
    #     save_video(
    #         tensor=result[None],
    ...
    #         value_range=(-1, 1),
    #     )
    #     logging.info(f"Saved full video to {full_output_path}")

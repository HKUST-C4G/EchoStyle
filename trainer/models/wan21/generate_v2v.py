...
def get_video(info):
    ...
    src_video = src_vr.get_batch(frame_indices)
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        for d in data:
            prompt = d["prompt"]
            output_filename = os.path.splitext("/".join(d["video_path"].split("/")[-2:]))[0] + ".mp4"
            output_path = os.path.join(args.output_dir, output_filename)
            logging.info(
                f"[Validation] src_video:{d['src_video_path']}, ref_image:{d['vace_ref_image_paths']} prompt: {prompt}, "
            ...
            video, src_video = get_video(d)
            print(video.shape)
            vace_video, vace_mask, vace_ref_images = prepare_vace_test_conditions(**d, video=video, src_video=src_video, max_pixels=args.max_pixels)

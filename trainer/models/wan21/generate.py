...
def _parse_args():
    ...
    ############################
    parser.add_argument("--lora_path", type=str, default=None, help="The path to the lora file.")
...
def generate(args):
    ...
    if "t2v" in args.task or "t2i" in args.task:
        ...
        if rank == 0:
            cache_video(
                tensor=video[None],
                save_file=os.path.join(args.output_path,"out_video.mp4"),
                fps=cfg.sample_fps,
                nrow=1,
                normalize=True,
                value_range=(-1, 1),
        ...
        ####################################################################
        # Modified by wan_trainer to support more flexible I2V evaluation. #
        ####################################################################
        # if args.prompt is None:
        #     args.prompt = EXAMPLE_PROMPT[args.task]["prompt"]
        ...
        cnt = 0
        for d in data:
            first_frame_path, prompt = d["first_frame_path"], d["prompt"]

            if args.pattern and not re.match(args.pattern, first_frame_path):
                continue
            ...
            output_filename = os.path.splitext("/".join(first_frame_path.split("/")[-2:]))[0] + ".mp4"
            output_path = os.path.join(args.output_dir, output_filename)
            logging.info(f"[Validation] first frame: {first_frame_path}, prompt: {prompt}, output: {output_path}")
        ...
        ######################################################################
        # Modified by wan_trainer to support more flexible FLF2V evaluation. #
        ######################################################################
        # if args.prompt is None:
        #     args.prompt = EXAMPLE_PROMPT[args.task]["prompt"]
        ...
        cnt = 0
        for d in data:
            first_frame_path, last_frame_path, prompt = d["first_frame_path"], d["last_frame_path"], d["prompt"]
            ...
            output_path = os.path.join(args.output_dir, output_filename)
            logging.info(
                f"[Validation] first frame: {first_frame_path}, last frame: {last_frame_path}, "
                f"prompt: {prompt}, output: {output_path}"
            )
        ...
        #####################################################################
        # Modified by wan_trainer to support more flexible VACE evaluation. #
        #####################################################################
        # if args.prompt is None:
        #     args.prompt = EXAMPLE_PROMPT[args.task]["prompt"]
        ...
        for d in data:
            ...
            vace_ref_image_paths = d.get("vace_ref_image_paths", None)
            prompt = d["prompt"]
            ...
            output_path = os.path.join(args.output_dir, output_filename)
            logging.info(
                f"[Validation] first frame: {first_frame_path}, key frame: {key_frame_path}, "
                f"last frame: {last_frame_path}, ref images: {vace_ref_image_paths}, prompt: {prompt}, "
            ...
            area = SIZE_CONFIGS[args.size]
            max_pixels = area[0] * area[1]

            vace_video, vace_mask, vace_ref_images = prepare_vace_test_conditions(**d, max_pixels=max_pixels)
            vace_video = [vace_video.cuda()]
            vace_mask = [vace_mask.cuda()]
            vace_ref_images = [x.cuda() for x in vace_ref_images] if vace_ref_images else None

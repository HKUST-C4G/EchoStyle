...
from .attention import flash_attention, ulysses
...
############################################################
# Modified by wan_trainer to apply rope at local sequence. #
############################################################
@torch.amp.autocast("cuda", enabled=False)
def rope_apply_sp(x, grid_sizes, freqs):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        sp_size = dist_ops.get_sequence_parallel_world_size()
        sp_rank = dist_ops.get_sequence_parallel_rank()
...
class WanSelfAttention(nn.Module):
    ...
    #########################################################
    # Modified by wan_trainer to support sequence parallel. #
    #########################################################
    def forward(self, x, seq_lens, grid_sizes, freqs):
        ...
        if self.enable_sp:
            transform = partial(rope_apply_sp, grid_sizes=grid_sizes, freqs=freqs)
            x = ulysses(
                q=q,
                k=k,
                v=v,
                ...
                group=dist_ops.get_sequence_parallel_group(),
                k_lens=seq_lens,
...
class echostyle(ModelMixin, ConfigMixin):
    r"""
    ...
    def enable_sequence_parallel(self):
        ...
        for m in self.modules():
            if isinstance(m, WanSelfAttention):
                m.enable_sequence_parallel()
    ...
    def forward(
        ...
        if y is not None:
            y = [self.patch_embedding(v.unsqueeze(0)) for v in y]
            ...
            #print("After concat, x shape:", x[0].shape)
        ...
        if self.enable_gc:
            for block in self.blocks:
                x = torch.utils.checkpoint.checkpoint(
                    block, x, e0, seq_lens, grid_sizes, self.freqs, context, context_lens, use_reentrant=False
                )
        else:
            # arguments
            kwargs = dict(
        ...
        #print("After unpatchify:", x[0].shape,grid_sizes)

...
class WanVace(WanT2V):
    ###############################################################################
    # Modified by wan_trainer to support passing created models and loading LoRA. #
    ###############################################################################
    def __init__(
        self,
        config,
        ...
        use_usp=False,
        t5_cpu=False,
        # Pass created models so we don't need to create them again.
        text_encoder=None,
        vae=None,
        clip=None,
        wan_model=None,
        ...
        if wan_model is None:
            ...
            if control_path:
                try:
                    finetuned_state_dict = torch.load(control_path, map_location='cpu')
                    if 'model_state_dict' in finetuned_state_dict:
                        finetuned_state_dict = finetuned_state_dict['model_state_dict']
                    elif 'model' in finetuned_state_dict:
                        finetuned_state_dict = finetuned_state_dict['model']

                    new_state_dict = OrderedDict()

                    # 遍历加载进来的 state_dict 的所有键值对
                    for k, v in finetuned_state_dict.items():
                        # 检查键名是否以 'module.' 开头
                        if k.startswith('module.'):
                            # 如果是，就去掉前缀 'module.' (它有7个字符)
                            name = k[7:] 
                    ...
                    # 现在，我们使用清理过键名的新 state_dict 来加载权重
                    finetuned_state_dict = new_state_dict
                    print("Loaded controlnet path.")
                    missing_keys, unexpected_keys = self.model.load_state_dict(finetuned_state_dict, strict=False)
                    if missing_keys:
                        print(f"   - 提示：模型中以下 {len(missing_keys)} 个参数未在 .pth 文件中找到，将保留预训练值:")
                        print(f"     {missing_keys}...") # 打印前5个看一眼
                    if unexpected_keys:
                        print(f"   - 警告：.pth 文件中以下 {len(unexpected_keys)} 个参数在模型中不存在，已被忽略:")
                        print(f"     {unexpected_keys}...") # 打印前5个看一眼

                except FileNotFoundError:
                    print(f"   错误：找不到微调权重文件 '{control_path}'，将仅使用预训练模型。")
                except Exception as e:
                    print(f"   错误：加载或应用微调权重时发生未知错误: {e}")
            ...
            self.sp_size = dist_ops.get_sequence_parallel_world_size()
    ...
    def vace_encode_frames(self, frames, ref_images, masks=None, vae=None):
        #print("vae:",vae,"frames", frames[0].shape)
    ...
    def mp_worker(self, gpu, gpu_infer, pmi_rank, pmi_world_size, in_q_list, out_q, initialized_events, work_env):
        try:
            ...
            while True:
                ...
                # evaluation mode
                with torch.amp.autocast("cuda", dtype=param_dtype), torch.no_grad(), no_sync():
                    if sample_solver == "unipc":
                        sample_scheduler = FlowUniPCMultistepScheduler(

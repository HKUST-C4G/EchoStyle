...
def _prepare_train_conditions(
    ...
    **kwargs,
    ...
    input_video = src_video.clone()
    #vace_mask = _get_vace_mask(video.shape)
    ref_images = _get_vace_ref_images(vace_ref_image_paths, video.shape[-2:])
    return input_video, ref_images


def prepare_test_conditions(
    #vace_modes: List[str] = None,
...
class v2vDataset(BaseDataset):
    def get_data(self, idx):
        ...
        input_video, ref_images = _prepare_train_conditions(
            **self.data_infos[idx], video=data["video"], src_video=data["src_video"]
        )
        ...
        data.update(dict(input_video=input_video, ref_images=ref_images))
        return data
    ...
    def cache_fn(self, idx, vae, text_encoder, *args):
        ...
        input_video = data["input_video"].cuda()
        #vace_mask = data["vace_mask"].cuda()
        ref_images = [x.cuda() for x in data["ref_images"]] if data["ref_images"] else None
        prompt = data["prompt"]

        context = text_encoder([prompt], "cuda")[0]  # [70, 4096]

        video_latent = _encode_frames([video], [ref_images], None, vae)[0]  # [16, 22, 138, 102] 
        ref_latent = _encode_frames([input_video], [ref_images], None, vae)[0]  #video的context

        # z0 = _encode_frames([vace_video], [vace_ref_images], [vace_mask], vae) #video的context
        # m0 = _vace_encode_masks([vace_mask], [vace_ref_images])
        # z = torch.cat([z0[0], m0[0]], dim=0)  # [96, 22, 138, 102]

        save_data = {"context": context, "video_latent": video_latent, "ref_latent": ref_latent}

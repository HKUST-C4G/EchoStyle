...
class Wanv2vDataset(BaseDataset):
    ...
    def cache_fn(self, idx, vae, text_encoder, cache_path, *args):
        ...
        tep_guide_video = src_video.clone()
        tep_guide_video[:, :16] = video[:, :16]
        #print(" video-shape:", video.shape, " src_video-shape:", src_video.shape)

        prompt = data["prompt"]
        #print("prompt:", prompt)
        context = text_encoder([prompt], "cuda")[0]  # [n, 4096]?
        rough_prompt = prompt.split("。")[0] + "。" #取句号前半部分作为粗略描述
        #print("rough_prompt:", rough_prompt)
        r_context= text_encoder([rough_prompt], "cuda")[0]
